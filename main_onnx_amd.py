...
def main():
    ...
    while win32api.GetAsyncKeyState(ord(aaQuitKey)) == 0:
        ...
        if len(targets) > 0:
            ...
            # Get the state of the Steam Controller
            events = inputs.get_gamepad()

                # Process the input events
            for event in events:
                if event.ev_type == "Absolute" and event.ev_code == "ABS_X":
                    # Left joystick moved in the X direction
                    move_x = event.ev_value / 32767.0
                elif event.ev_type == "Absolute" and event.ev_code == "ABS_Y":
                    # Left joystick moved in the Y direction
                    move_y = event.ev_value / 32767.0
            # Check if the move_x and move_y variables are still None
            ...
            # Moving the mouse
            #if win32api.GetKeyState(0x14):
                #win32api.mouse_event(win32con.MOUSEEVENTF_MOVE, x_new, y_new, 0, 0)
